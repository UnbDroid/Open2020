...
def Align():   #em desenvolvimento
	...
	erro,angIni=sim.simxGetObjectOrientation(glob.clientID,glob.robo,-1,sim.simx_opmode_streaming)
	while(erro != 0):
		erro,angIni=sim.simxGetObjectOrientation(glob.clientID,glob.robo,-1,sim.simx_opmode_streaming)
	gamma=(angIni[2]*180)/(np.pi)
	if(gamma > 45 and gamma < 125):
		directIni = EAST
	elif((gamma < 179 and gamma > 0) or (gamma < -125 and gamma < 0)):
		directIni = NORTH
	elif(gamma <= 45 and gamma >= -45):
		directIni = SOUTH
	else:
		directIni = WEST
	move.MoveForward(v)
